...
#' financial index. The data is fetched from a remote CSV file, filtered,
#' and cleaned to provide relevant information about constituents.
#'
...
#' index from ETF sites, then sends an HTTP GET request to download the
#' CSV file, and processes the CSV file to extract equity constituents.
#'
...
#'   \item{currency}{The currency in which the equity is traded,
...
download_data_constituents <- function(index) {
  ...
  if (!is.null(response)) {
    if (response$status_code != 200) {
      cli::cli_abort(
        paste(
          "Failed to download data for index {.arg index}.",
      ...
      constituents_raw <- suppressWarnings(httr2::resp_body_string(response)) |>
        textConnection() |>
        read.csv(
          skip = supported_indexes$skip[supported_indexes$index == index]
        )

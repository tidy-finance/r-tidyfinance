...
test_that("download_factor_library_returns_ids() aborts when no IDs match the grid", {
...
test_that("download_factor_library_returns_ids() aborts when a parquet URL is missing for a portfolio ID", {
  grid_rows <- tibble::tibble(
    id = 1L,
    sorting_variable = "sv_me",
    sorting_variable_lag = "3m"
  ...
  mock_files <- function(organization, dataset) {
    if (dataset == "factor-library-grid") {
      ...
      tibble::tibble(path = character(0), size = numeric(0), url = character(0))
    }
  }

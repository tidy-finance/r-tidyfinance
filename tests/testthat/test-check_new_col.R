...
test_that("check_new_col warns when ret_excess would be overwritten", {
  # Match both branches of the message: "<caller> overwrites ..." and
  # "Overwriting ..." (the latter fires when caller_call()/call_name()
  # resolves to NULL, e.g. under anonymous evaluation contexts).
  expect_warning(
...
test_that("check_new_col warns when multiple portfolio columns would be overwritten", {
...
test_that("check_new_col uses the 'Overwriting' branch when caller has no name", {
  # Wrapping the call in an anonymous function gives a caller frame whose
  # call has a function expression as its head rather than a symbol, so
  # `rlang::call_name(rlang::caller_call())` returns NULL and the message
  # falls into the else branch ("Overwriting ...") instead of the named
  # "<fn> overwrites ..." form.
  expect_warning(
...
test_that("check_new_col emits exactly one warning even for multiple overlapping cols", {
...
test_that("check_new_col warns only for existing columns, not new ones in the same call", {
  # bm exists, bm_lag does not
...
test_that("check_new_col returns the input data unchanged when overlap triggers a warning", {
...
test_that("add_lagged_columns warns when .upper already exists and max_lag is used", {
...
test_that("compute_long_short_returns warns when ..date already exists in data", {
...
test_that("filter_sorting_data warns when .size_cutoff already exists in data", {
...
test_that("join_lagged_values warns when new_data already has .year and ff_adjustment is TRUE", {
...
test_that("compute_portfolio_returns warns when portfolio already exists in data", {
